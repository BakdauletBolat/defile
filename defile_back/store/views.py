...
class OrderListApiView(ListAPIView):
    ...
    def get_queryset(self):
        return self.queryset.filter(owner=self.request.user).exclude(status='ожидает')
...
class GetOrderToUser(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if request.user.is_authenticated:
            baskets = Order.objects.filter(owner=request.user, status='ожидает')

            if len(baskets) > 0:     
                return JsonResponse(OrderSerializer(baskets.first(), context={'request': request}).data)
            else:
        ...
        if request.user.is_authenticated:
            try:
                ...
                OrderItem.objects.create(
                    product_id=product_id, order=basket, qty=qty,size_id=size)
